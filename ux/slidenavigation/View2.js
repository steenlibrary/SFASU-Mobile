 ...
 */
Ext.define('Ext.ux.slidenavigation.View2', {
    ...
    config: {
        ...
        list: {
            width: 250,
            maxDrag: null,
            itemTpl: '<img src="{icon}" width="20px"/> {title}',
            grouped: true,
            items: [{
                xtype: 'toolbar',
                docked: 'top',
    ...
    onSelect: function(list, item, eOpts) {
        ...
        if (me._cache[index] == undefined) {
            ...
            if (Ext.isFunction(item.raw.handler)) {
				...
				//console.log(me._cache);
				for(var cached in me._cache) {
					console.log(cached);
					if((cached != index) && (me._cache[cached] != undefined)) {
						me._cache[cached].destroy();
						me._cache[cached] = undefined;
					}
				}
